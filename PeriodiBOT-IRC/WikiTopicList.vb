...
Public Class WikiTopicList
    ...
    Sub New(ByRef wbot As Bot)
        WorkerBot = wbot
    End Sub

    Function UpdateTopics() As Boolean
            Try
            Dim topicpage As Page = WorkerBot.Getpage(WPStrings.TopicPageName)
        ...
        Dim UpdateText As String = "<span style=""color:#0645AD"">►</span> Actualizado al " & Integer.Parse(UpdateDate.ToString("dd")).ToString & UpdateDate.ToString(" 'de' MMMM 'de' yyyy 'a las' HH:mm '(UTC)'", New System.Globalization.CultureInfo("es-ES")) _
                & " por [[Usuario:" & WorkerBot.UserName & "|" & WorkerBot.UserName & "]] sobre un total de " & scannedPages.ToString & " páginas de archivo." & Environment.NewLine
    ...
    Private Function GetTopicGroups() As SortedDictionary(Of String, List(Of String))
        Dim GroupsPage As Page = WorkerBot.Getpage(WPStrings.TopicGroupsPage) 'Inicializar página de grupos
    ...
    Private Function GetAllTopicThreads(ByRef inclusions As Integer) As SortedList(Of String, WikiTopic)
        ...
        Dim pages As String() = WorkerBot.GetallInclusions(WPStrings.TopicTemplate) 'Paginas que incluyen la plantilla de tema.
        For Each p As String In pages 'Por cada página que incluya la plantilla tema, no se llama a GetallInclusionsPages por temas de memoria.
            Topiclist = GetTopicsOfpage(WorkerBot.Getpage(p), Topiclist) 'Añadir nuevos hilos
        Next
    ...
    Function BiggestThreadsEver() As Boolean
        ...
        Dim Pages As String() = WorkerBot.PrefixSearch("Wikipedia:Café/")
        Dim TotalThreadCount As Integer = 0

        For Each CPage As String In Pages
            Dim wPage As Page = WorkerBot.Getpage(CPage)
        ...
        Dim tPage As Page = WorkerBot.Getpage(WPStrings.LongestThreads)
